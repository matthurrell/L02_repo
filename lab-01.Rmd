...
NA stands for Not Available, this appears because NA's appear for some some attractions in n_2021 leading to no calculation being able to be made as NA is not a number.
...
The highest values for n_2022 are extremely high and the lowest are not much lower than the median leading to these values making the mean far larger than the median
...
visitors_with_nations %>%
  group_by(nation) %>% 
  summarise(attractions_per_nation = n())
...
free_nation <- visitors_with_nations %>%
  mutate(free_admission = case_when(
    admission == "Free" ~ "Free",
    admission == "Members" ~ "Members",
    TRUE ~ "Charged"
  ))

str(free_nation)
    
free_only <- free_nation %>% 
  filter(free_admission == "Free")

#table(free_only$free_admission)
#, .groups = "drop"

free_admissions_only <- free_only %>% 
  group_by(nation) %>%
  summarise(Num_Free_admission = n(), .groups = "drop")

print(free_admissions_only)
```
